...
Ext.DatePicker = Ext.extend(Ext.Component, {
    ...
    /**
     * @cfg {String} todayTip
     * 选择当前日期的按钮的快捷提示（默认为 "{current date} (Spacebar)"）
     */
    todayTip : "{0} (Spacebar)",
    /**
     * @cfg {Date} minDate
    ...
    /**
     * @cfg {String} format
     * 默认的日期格式化字串，可以被覆写以实现本地化支持。格式化字串必须为 {@link Date#parseDate} 中记载的有效格式（默认为 'm/d/y'）。
     */
    format : "m/d/y",
    /**
     ...
     * 值为 true 时强制日期选择器显示在 viewport 中（默认为 true）
     */
    constrainToViewport : true,
    /**
     * @cfg {Array} monthNames
     * 月份名称数组，可以被覆写以实现本地化支持（默认为 Date.monthNames）
     */
