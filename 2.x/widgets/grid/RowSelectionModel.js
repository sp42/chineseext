...
Ext.grid.RowSelectionModel = function(config){
    ...
    this.addEvents(
	     ...
	     * 当选区改变时触发。Fires when the selection changes
	     * @param {SelectionModel} this
	     */
	    "selectionchange",
        /**
	     * @event beforerowselect
	     * 当行（row）是选中又被选择触发，返回false取消。Fires when a row is being selected, return false to cancel.
	     * @param {SelectionModel} this
        ...
        /**
	     * @event rowselect
	     * 当行（row）被选中时触发。Fires when a row is selected.
	     * @param {SelectionModel} this
        ...
        /**
	     * @event rowdeselect
	     * 当行（row）反选时触发。Fires when a row is deselected.
	     * @param {SelectionModel} this
...
Ext.extend(Ext.grid.RowSelectionModel, Ext.grid.AbstractSelectionModel,  {
     ...
     * True表示为同时只能选单行（默认false）。True to allow selection of only one row at a time (defaults to false)
     */
    singleSelect : false,
     ...
     * @param {Boolean} keepExisting (optional) （可选的）true表示为保持当前现有的选区。True to keep existing selections
     */
    ...
    /**
     * 对选区执行传入函数。如果函数返回false，枚举将会中止，each函数就会返回false，否则返回true。
    ...
    /**
     * 选取多行。Selects multiple rows.
     * @param {Array} rows 要选取行的索引的集合。Array of the indexes of the row to select
     * @param {Boolean} keepExisting (optional) （可选的）表示为保持现有的选区。True to keep existing selections (defaults to false)
     */
    ...
    /**
     * 选取某个范围内的行（rows）。所有在startRow和endRow之间的行都会被选中。Selects a range of rows. All rows in between startRow and endRow are also selected.
     ...
     * @param {Boolean} keepExisting (optional)（可选的）表示为保持现有的选区。True to retain existing selections
     */
    ...
    /**
     * 反选某个范围内的行（rows）。所有在startRow和endRow之间的行都会被选反。Deselects a range of rows. All rows in between startRow and endRow are also deselected.
     * @param {Number} startRow 范围内的第一行之索引。The index of the first row in the range
     * @param {Number} endRow 范围内的最后一行之索引。The index of the last row in the range
     */
