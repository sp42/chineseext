...
Ext.grid.GridView = function(config){
    ...
    this.addEvents(
         ...
         * @param {Number} rowIndex 要移除行的索引。
         * @param {Ext.data.Record} record 已移除的记录。
	     */
	    "beforerowremoved",
	    /**
         ...
         * 插入行之前触发。
         * @param {Ext.grid.GridView} view
         * @param {Number} firstRow 被插入第一行的索引
         * @param {Number} lastRow 被最后一行的索引
	     */
	    "beforerowsinserted",
	    /**
         * @event beforerefresh
         ...
         * 当有行被移除时触发
         * @param {Ext.grid.GridView} view
         * @param {Number} rowIndex 被移除行的索引
         * @param {Ext.data.Record} record 被移除的Record对象
	     */
	    "rowremoved",
	    /**
         * @event rowsinserted
         * 当有行被插入时触发
         * @param {Ext.grid.GridView} view
         * @param {Number} firstRow 插入所在位置的索引
         * @param {Number} lastRow 插入位置上一行的索引
	     */
	    "rowsinserted",
	    /**
         * @event rowupdated
         * 当有一行的内容被更新后触发。
         * @param {Ext.grid.GridView} view
         * @param {Number} firstRow 更新行的索引
         * @param {Ext.data.record} record 更新后行的Record对象
	     */
	    "rowupdated",
	    /**
         * @event refresh
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
     ...
     * @param {Record} record 当然行所关联的{@link Ext.data.Record}对象
     * @param {Number} index 行索引
     * @param {Object} rowParams 
     * 传入到行模板（row template）的配置项对象，以便渲染过程中可控制每行的部分是怎么显示的（可选的）。 
     * 注意该对象只会在{@link #enableRowBody} = true时有效，不然的话会被忽略。对象可以包含下例的属性：
     * <ul>
     * <li><code>body</code> : String <div class="sub-desc">渲染成为单元格内容部分的HTML判断（默认为''）。</div></li>
     * <li><code>bodyStyle</code> : String <div class="sub-desc">加入到行所在TR元素的CSS样式（默认为''）</div></li>
     * <li><code>cols</code> : Number <div class="sub-desc">
     * 列的数量，与body 行的TDcolspan属性紧密联系（默认为当前grid的列数）。</div></li>
     * </ul>
     * @param {Store} ds The {@link Ext.data.Store} this grid is bound to 与此GRID所绑定的{@link Ext.data.Store}对象
     * @method getRowClass
     * @return {String} a CSS class name to add to the row. 行新加入的样式类
     */
     ...
     * True表示为在每一数据行的下方加入一个TR的元素，为行部分提供位置。应使用{@link #getRowClass}方法的rowParams的配置自定义行部分。
     */
     
    /**
     * @cfg {String} emptyText 
     * Default text to display in the grid body when no rows are available (defaults to '').
     * 当GRID没有一行可供显示时出现在body的提示文本（默认为""）。
     */
     ...
     * True表示为<b>当GRID创建后</b>自动展开各列，自适应整个GRID。
     */
    autoFill: false,
    /**
	 ...
	 * @param {Number} row 定位单元格的行索引
	 * @param {Number} col 单元格的列索引
	 * @return {HtmlElement} 坐标所在的&lt;TD>元素
	 */
    ...
    /**
     * 刷新grid UI
     * @param {Boolean} headersToo （可选的）True表示为也刷新头部
     */
