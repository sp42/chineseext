 ...
 * 注意要产生动画效果必须依赖{@link Ext.Fx}的包方可正确调用功能。
...
--------- --------  ---------------------------------------------
{@link Ext.Fx#duration duration}  .35      动画持续的时间（单位：秒）。 The duration of the animation in seconds
{@link Ext.Fx#easing easing}    easeOut   退却（收尾）的方法。The easing method
{@link Ext.Fx#callback callback}  none     动画完成之后执行的函数。 A function to execute when the anim completes
{@link Ext.Fx#scope scope}     this      回调函数的作用域。The scope (this) of the callback function
</pre>
 *
 * <pre><code>
...
var El = Ext.Element = Ext.extend(Object, {
    ...
    /**
     * 加入一个事件处理函数（{@link #addListener}的简写方式）Appends an event handler to this element.  The shorthand version {@link #on} is equivalent.
     * @param {String} eventName 事件处理函数的名称The name of event to handle.
     * @param {Function} fn 事件处理函数The handler function the event invokes. This function is passed
     ...
     * </ul>
     * @param {Object} scope （可选的）函数的作用域（默认这个元素）(optional) The scope (<code><b>this</b></code> reference) in which the handler function is executed.
