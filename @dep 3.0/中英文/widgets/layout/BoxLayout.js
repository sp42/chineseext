 ...
 * <p>HBoxLayout和VBoxLayout的基类。一般情况下不会直接使用。Base Class for HBoxLayout and VBoxLayout Classes. Generally it should not need to be used directly.</p>
 */
Ext.layout.BoxLayout = Ext.extend(Ext.layout.ContainerLayout, {
    /**
     * @cfg {Object} defaultMargins
     * <p>设置外边距（margins）。如果不制定每个子项的<tt>margins</tt>属性，那么就会使用默认的margins。If the individual contained items do not have a <tt>margins</tt>
     * property specified, the default margins from this property will be
     * applied to each item.</p>
     * <br><p>This property may be specified as an object containing margins
     * to apply in the format:指定margin的格式如下：</p><pre><code>
{
    top: (top margin),
    right: (right margin),
     ...
     * with each value matches the way CSS processes margin values:该属性接纳的格式也可以是字符串的，其格式规定为空格隔开，数字类型的margin值。各个方向的排列顺序为CSS的顺序：</p>
     * <div class="mdetail-params"><ul>
     * <li>如果只是一个值，就是各个方向都是这个值。If there is only one value, it applies to all sides.</li>
     * <li>如果有两个值，那么第一个值代表是上下方向的值，第二值则是左右方向的值。If there are two values, the top and bottom borders are set to the
     * first value and the right and left are set to the second.</li>
     * <li>如果有三个值，第一个值是顶部的值，第二个值是左右两边的值，第三个值是底部的值。If there are three values, the top is set to the first value, the left
     * and right are set to the second, and the bottom is set to the third.</li>
     * <li>如果有四个值，分别代表就是顶部、右边、底部和左边的值（顺时针方向）。If there are four values, they apply to the top, right, bottom, and
     * left, respectively.</li>
     * </ul></div>
     * <p>默认为：Defaults to:</p><pre><code>
    ...
    /**
     * @cfg {String} padding
     * <p>设置内边距（margins）。Sets the padding to be applied to all child items managed by this layout.</p>
     * <p>该属性接纳的格式也可以是字符串的，其格式规定为空格隔开，数字类型的margin值。各个方向的排列顺序为CSS的顺序：This property must be specified as a string containing
    ...
    /**
     * @private
     * 计算子box的布局并缓存这些计算值。子类的话就可以继续使用。Runs the child box calculations and caches them in childBoxCache. Subclasses can used these cached values
     * when laying out
     */
    onLayout: function(container, target) {
         ...
         * @type Object
         * 布局目标上一次计算过的缓存。如果不是BoxLayout的子类onLayout，一般也不会访问该私有的值。Private cache of the last measured size of the layout target. This should never be used except by
         ...
         * @type Array
         * 这是一个数组，数组内容是每个可视渲染项其高度、宽度和top、left。每一次计算都不同。Array of the last calculated height, width, top and left positions of each visible rendered component
    ...
    /**
     * 对每个子组件调整大小和从新布局。Resizes and repositions each child component
     * @param {Array} boxes 箱子的大小。The box measurements
     */
    updateChildBoxes: function(boxes) {
        for (var i = 0, length = boxes.length; i < length; i++) {
            ...
            // NaN的不安排位置。Don't set positions to NaN
    ...
    /**
     * @private
     * 由onRender调用，在子组件大小变化之前，也在位置调整之前。Called by onRender just before the child components are sized and positioned. This resizes the innerCt
    ...
    /**
     * @private
     * 返回那些已经渲染并且可视了的子项。Returns all items that are both rendered and visible
     * @return {Array} All 命中的项集合。matching items
     */
 ...
 * <p>垂直方向分配子项的布局。可选地，这种布局会把有设定<code>flex</code>配置的子项划分垂直的空间。这种布局可以用子项的{@link #align}来设置子项的宽度。A layout that arranges items vertically down a Container. This layout optionally divides available vertical
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
    /**
     * @cfg {String} align
     * 设置容器内的子项是怎么对齐的。该属性可接纳的配置项有：Controls how the child items of the container are aligned. Acceptable configuration values for this
     * property are:
     * <div class="mdetail-params"><ul>
     * <li><b><tt>left</tt></b> : <b>默认的Default</b><div class="sub-desc">居左，从容器的<b>left</b>开始水平对齐。child items are aligned horizontally
     * at the <b>left</b> side of the container</div></li>
     * <li><b><tt>center</tt></b> : <div class="sub-desc">居中，从容器的<b>mid-width</b>开始垂直对齐。child items are aligned horizontally at the
     * <b>mid-width</b> of the container</div></li>
     * <li><b><tt>stretch</tt></b> : <div class="sub-desc">拉伸子项以填充容器的水平宽度。child items are stretched horizontally to fill
     * the width of the container</div></li>
     * <li><b><tt>stretchmax</tt></b> : <div class="sub-desc">拿最宽的那个子项拉伸，适应容器的水平宽度。child items are stretched horizontally to
    ...
    /**
     * @cfg {String} pack
     * 控制容器里面的子项是如何停靠的。可接纳的值有以下几种：Controls how the child items of the container are packed together. Acceptable configuration values
     * for this property are:
     * <div class="mdetail-params"><ul>
     * <li><b><tt>start</tt></b> : <b>默认的Default</b><div class="sub-desc">居顶，从容器的<b>top</b>停靠子项。child items are packed together at
     * <b>top</b> side of container</div></li>
     * <li><b><tt>center</tt></b> : <div class="sub-desc">居中，也就是子项都从容器的<b>mid-height</b>上开始停靠。child items are packed together at
     * <b>mid-height</b> of container</div></li>
     * <li><b><tt>end</tt></b> : <div class="sub-desc">居底，从容器的<b>底部</b>边边开始往上摆子组件。child items are packed together at <b>bottom</b>
    ...
    /**
     * @cfg {Number} flex
     * 该配置项是设置在<b>子项</b>的配置项，在容器其中。每个子项<b>相对的</b><tt>flex</tt>值都会与全体子项<tt>flex</tt>累加的值相比较，根据此结果，处理每个子项的<tt>flex</tt>最后是多少。如果是<tt>flex = 0</tt>的子项或<tt>flex = undefined</tt>的子项就是不自伸缩处理，即采用最初的尺寸。This configuation option is to be applied to <b>child <tt>items</tt></b> of the container managed
    ...
    /**
     * @private
     * 参阅父类文档。See parent documentation
     */
 ...
 * @extends Ext.layout.BoxLayout
 * <p>水平方向分配子项的布局。可选地，这种布局会把有设定<code>flex</code>配置的子项划分水平的空间。这种布局可以用子项的{@link #align}来设置子项的高度。A layout that arranges items horizontally across a Container. This layout optionally divides available horizontal
 * space between child items containing a numeric <code>flex</code> configuration.</p>
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
    /**
     * @cfg {String} align
     * 控制容器内每个子项是如何对齐的。可接纳的值有以下几种：Controls how the child items of the container are aligned. Acceptable configuration values for this
     * property are:
     * <div class="mdetail-params"><ul>
     * <li><b><tt>top</tt></b> : <b>默认的 Default</b><div class="sub-desc">顶部对齐，从容器的<b>top</b>开始垂直对齐。child items are aligned vertically
     * at the <b>top</b> of the container</div></li>
     * <li><b><tt>middle</tt></b> : <div class="sub-desc">居中对齐，从容器的<b>middle</b>开始垂直对齐。child items are aligned vertically in the
     * <b>middle</b> of the container</div></li>
     * <li><b><tt>stretch</tt></b> : <div class="sub-desc">拉伸子项以填充容器的垂直高度。child items are stretched vertically to fill
     * the height of the container</div></li>
     * <li><b><tt>stretchmax</tt></b> : <div class="sub-desc">拿最高的那个子项拉伸，适应容器的垂直高度。child items are stretched vertically to
    ...
    /**
     * @private
     * 请参阅父类文档。See parent documentation
     */
    updateInnerCtSize: function(tSize, calcs) {
     ...
     * for this property are:
     * <div class="mdetail-params"><ul>
     * <li><b><tt>start</tt></b> : <b>默认的 Default</b><div class="sub-desc">居左，从左边开始排列内容。child items are packed together at
     * <b>left</b> side of container</div></li>
     * <li><b><tt>center</tt></b> : <div class="sub-desc">居中，也就是从容器的<b>mid-width</b>开始停靠内容。child items are packed together at
     * <b>mid-width</b> of container</div></li>
     * <li><b><tt>end</tt></b> : <div class="sub-desc">居右，从容器的<b>右边</b>开始停靠内容。child items are packed together at <b>right</b>
     * side of container</div></li>
     * </ul></div>
     */
    /**
    ...
    calculateChildBoxes: function(visibleItems, targetSize) {
        var visibleCount = visibleItems.length,
            ...
            for (i = 0; i < visibleCount; i++) {
                ...
                //为对齐求得最高高度，强制让没有数值高度的子容器进行布局。 Max height for align - force layout of non-layed out subcontainers without a numeric height
                if (!Ext.isNumber(childHeight)) {
                    if (canLayout) {
                        child.doLayout();
                    }
                ...
                //将每个子项的大小缓存起来。 cache the size of each child component
                boxes.push({
                    component: child,
                    height   : childHeight || undefined,
                    width    : childWidth || undefined
            ...
            //flex项的宽度。the width available to the flexed items
            ...
            //用于计算Flex宽度的临时变量。temporary variables used in the flex width calculations below
            var remainingWidth = availableWidth,
                remainingFlex  = totalFlex;

            //计算flex的宽度、left/top。calculate the widths of each flexed item, and the left + top positions of every item
            for (i = 0; i < visibleCount; i++) {
                child = visibleItems[i];
                calcs = boxes[i];
